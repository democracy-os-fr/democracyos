...
class ReactImageUploadComponent extends React.PureComponent {
  ...
  */
  handleChange (e) {
    ...
    this.removeImage(pics[0].url.split('\\').pop().split('/').pop())
    console.log('pics is ' + pics)
    console.log(this.state.pictures)
    this.props.onChanged(pics[0].url)
  }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      reader.onload = (function () {
        return function (e) {
          let picture = {
            name: f.name,
            url: window.location.origin + '/uploads/' + f.name,
    ...
    return this.state.pictures.map((picture, index) => {
      return (
        <div key={index} className='uploadPictureContainer'>
          <div className='deleteImage' onClick={() => this.removeImage(picture.name)}>X</div>
      ...
      return (
        <div className='coverWrapper'>
          ...
          <div className='coverContainer' style={{
            backgroundImage: 'linear-gradient(rgba(0,0,0, 0.2), rgba(0,0,0, 0.2)), url("' + picture.url + '")'
          }} />
        </div>
      )
    }
  ...
  render () {
    return (
      <div className={'fileUploader ' + this.props.className} style={this.props.style}>
        { this.props.withPreview && this.props.singleImage && this.renderCoverPreview() }
        { this.props.singleImage && this.props.withUrl && (
          ...
          <div className='input-group'>
            <input type='text' id='url' className='form-control'
              ...
              placeholder={window.location.origin + '/uploads/...'}
