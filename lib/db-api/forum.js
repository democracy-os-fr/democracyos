...
exports.all = function all (options, fn) {
  ...
  if (options) {
      ...
      query.find({
        $or: [
          { owner: options['privileges.canChangeTopics'] },
          {
            permissions: {
              $elemMatch: {
                user: options['privileges.canChangeTopics'],
    ...
    if (options['privileges.canView']) {
      query.find({
        $or: [
          { owner: options['privileges.canView'] },
          {
            permissions: {
              $elemMatch: { user: options['privileges.canView'] }
            }
          },
  ...
  if (!options || (!options.owner && !options['privileges.canChangeTopics'] && !options['privileges.canView'])) {
