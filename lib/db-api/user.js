...
exports.search = function search(query, fn) {
  ...
  .select('firstName lastName fullName createdAt updatedAt email avatar profilePictureUrl notifications roles activities location age custom')
  .exec(function(err, users) {
    if (err) {
      log('Found error: %j', err);
      return fn(err);
...
exports.getByEmail = function search(email, fn) {
  ...
  .select('firstName lastName fullName createdAt updatedAt email avatar profilePictureUrl notifications emailValidated roles activities location age custom')
  .exec(function(err, user) {
    if (err) {
...
exports.lookup = function lookup(ids, fn) {
  ...
  .select('firstName lastName fullName createdAt updatedAt email avatar staff status profilePictureUrl notifications roles activities location age custom')
  .exec(function(err, users) {
...
exports.expose.confidential = function(user){
  return expose('id firstName lastName fullName createdAt updatedAt displayName hasPassword profileType email roles activities location age avatar profilePictureUrl staff status notifications locale custom')(user);
}
...
exports.expose.ordinary = function(user){
  return expose('id fullName createdAt updatedAt displayName avatar profilePictureUrl profileType email roles activities location age staff status notifications locale custom')(user);
}
