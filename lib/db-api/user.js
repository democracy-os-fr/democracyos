...
var log = debug('democracyos:db-api:user')
...
exports.getByEmail = function search (email, fn) {
  ...
  User.findOne({ email: email })
    .select('firstName lastName fullName username email avatar profilePictureUrl notifications emailValidated extra')
    .exec(function (err, user) {
      if (err) {
        log('Found error: %j', err)
        return fn(err)
...
exports.findByWhitelist = function findByWhitelist (whitelist, cb) {
  log('Finding users for whitelist %o', whitelist)

  if (isEmpty(whitelist)) {
    var err = new Error('whitelist is empty')
    log('Found error: %j', err)
    return cb(err)
  }
  ...
  switch (whitelist.type) {
    case 'email':
      query = { email: whitelist.value }
      break
    case 'domain':
      query = { email: new RegExp('@' + whitelist.value + '$') }
      break
  }
  ...
  User.find(query)
    ...
    .exec(function (err, users) {
      ...
      log('Found users %s for whitelist %o', pluck(users, 'id'), whitelist)
      cb(null, users || [])
    })
}
...
exports.create = function create (u, fn) {
  log('Creating new user %j', u)
  let o = new User(u)
