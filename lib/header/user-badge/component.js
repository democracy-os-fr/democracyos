...
export class UserBadge extends Component {
  static links = [
    config.frequentlyAskedQuestions
      ...
      : { label: t('help.faq.title'), path: '/help/' }
  ].filter((p) => p)
    ...
    return (
      <div className={classes.join(' ')} onClick={this.toggleMenu}>
        <button className='header-link'>
          ...
          <span className='name hidden-xs-down'>{userAttrs.username}</span>
        ...
        <ul
          ...
          <li>
            <a href='https://pad.f0rk.fr/s/rk8TWAZRX'>
              {t('help.legal.title')}
            </a>
          </li>
          <li>
