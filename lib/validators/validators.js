...
export default {
  ...
  domain: (val, option, fn) => {
    val = val.trim()
    fn(!regexps.domain.test(val) && val !== '' ? t('validators.invalid.domain') : null)
  },

  domains: (val, option, fn) => {
    val = val.split(',').map((e) => e.trim().toLowerCase()).join(',')
    fn(!regexps.domains.test(val) && val !== '' ? t('validators.invalid.domain') : null)
  },
  ...
  hexColor: (val, option, fn) => {
    fn(val.trim() !== '' && !regexps.hexColor.test(val) ? t('validators.color.hex') : null)
  },

  postalFR: (val, option, fn) => {
    fn(val.trim() !== '' && !regexps.postal.fr.test(val) ? t('user.extra.postal.validator') : null)
  }
