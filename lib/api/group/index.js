...
var log = new Log('democracyos:group-api')
...
app.post('/group/create', function (req, res, next) {
  ...
  var owners = req.body.owners ? req.body.owners : []
  var users = req.body.users ? req.body.users : []

  if (!contains(owners, req.user._id)) {
    req.body.owners.push(req.user._id)
  }

  if (!contains(users, req.user._id)) {
...
app.get('/group/search/:name', function (req, res) {
  var name = req.params.name
  log('Request /group/search/%s', name)
