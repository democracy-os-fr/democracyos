...
app.post('/settings/profile', restrict, function (req, res) {
  ...
  if (req.body.updateOnNextLogin) {
    user.profiles = Object.assign(user.profiles, {
      updateOnNextLogin: req.body.updateOnNextLogin
    })
  }
  ...
  for (var key of ['age', 'job', 'org', 'postal']) {
    if (config.extra.user[key] && req.body[key]) {
      if (!user.hasOwnProperty('extra')) user.extra = {}
      user.extra[key] = req.body[key]
    }
  }
