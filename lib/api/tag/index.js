...
app.get('/tag/all', function (req, res) {
  ...
  api.tag.all(function (err, tags) {
    ...
    res.status(200).json(tags.map(expose('id hash name color icon image createdAt')))
  })
})
...
app.get('/tag/:id', function (req, res) {
  ...
  api.tag.get(req.params.id, function (err, tagDoc) {
    ...
    var keys = [
      'id hash name color icon image createdAt'
    ].join(' ')
...
app.post('/tag/generate', staff, function (req, res, next) {
  log('Request /tag/generate')

  var tag = {
    name: generator.words(1),
    color: generator.colors(1),
    icon: generator.icons(1)
  }

  api.tag.create(tag, function (err, tagDoc) {
    if (err) return _handleError(err, req, res)
    var keys = [
      'id hash name color icon image createdAt'
    ].join(' ')
