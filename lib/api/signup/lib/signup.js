...
exports.doSignUp = function doSignUp (profile, meta, callback) {
  const strategySignup = () => {
    ...
    for (var key of ['age', 'job', 'org', 'postal']) {
      if (config.extra.user[key] && profile[key]) {
        if (!profile.hasOwnProperty('extra')) profile.extra = {}
        profile.extra[key] = profile[key]
      }
    }

    var user = new User(profile)

    log('new user [%s] from Local signup [%s]', user, profile.email)
