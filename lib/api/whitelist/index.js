...
if (config.usersWhitelist) {
  ...
  }, findUsers, grantPermissions, function (req, res) {
    res.status(200).json(req.whitelists)
  })

  app.post('/whitelists/:id', staff, function (req, res, next) {
    ...
    api.whitelist.update(req.params.id, req.body, function (err, whitelist) {
      ...
      req.whitelists = [whitelist]
      next()
    })
  }, findUsers, grantPermissions, function (req, res) {
    res.status(200).json(req.whitelist.toJSON())
  })
...
function grantPermissions (req, res, next) {
  ...
  batch.end(function (result) {
    // result should be the active forum
    return next()
  })
}
