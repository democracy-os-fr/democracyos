...
app.get('/forum/all', function (req, res) {
  ...
  var limit = config.forumsPageLimit
...
app.post('/forum', middlewares.canCreateForum, restrict, maintenance, function (req, res) {
  var data = {
    ...
    creationReason: req.body.creationReason,
    name: req.body.name,
    ...
    featuredSection: ((req.body.featuredSection === 'on') || req.body.featuredSection),
    featured: ((req.body.featured === 'on') || req.body.featured === 'on'),
    publishedAt: null
  }
...
app.post('/forum/copy', middlewares.canCreateForum, restrict, maintenance, function (req, res) {
  log('Trying to duplicate forum %s with name: %s', req.body.source, req.body.name)
...
app.post('/forum/:id',
  ...
  function (req, res) {
    ...
    updateFeaturedSection(req, req.body)
    updateFeatured(req, req.body)
...
app.get('/forums/getByGroup/:groupId', function (req, res) {
  var groupId = req.params.groupId
  log('Request /forums in group/%s', groupId)

  api.forum.findByGroup(groupId, function (err, forums) {
    if (err) return _handleError(err, req, res)

    log('Serving forums %j', forums)
...
function updateFeaturedSection (req, data) {
  data.featuredSection = ((req.body.featuredSection === 'on') || req.body.featuredSection === 'on')
}
function updateFeatured (req, data) {
  data.featured = ((req.body.featured === 'on') || req.body.featured === 'on')
}
