...
let lasturl
function getUploadedFile (callback) {
  app.get('/api/v2/upload.files', function (req, res, next) {
    console.log(res)
    let result = res.json()
    let lastFileUploaded
    let last
    let files = result.files
    let creationDates = result.filesDates
    let X = new Date(creationDates[0].birthtime.toString())
    for (let i = 0; i < creationDates.length; i++) {
      if (X <= new Date(creationDates[i].birthtime.toString())) {
        X = new Date(creationDates[i].birthtime.toString())
        last = i
      }
    }
    lastFileUploaded = files[last]
    lasturl = result.url + lastFileUploaded
      // url = lasturl
    return callback(lasturl)
  })
}
function getFileUrl (lasturl) {
...
app.post('/forum', middlewares.canCreateForum, restrict, maintenance, function (req, res) {
  var data = {
    ...
    coverUrl: getUploadedFile(getFileUrl),
