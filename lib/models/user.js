...
const UserSchema = new Schema({
  ...
  profiles: {
    ...
    twitter: { type: Object },
    ldap: { type: Object },
    updateOnNextLogin: { type: Boolean, default: false }
  },
...
UserSchema.index({ 'notifications.new-comment': 1 })
...
/**
 * Get `displayName` from `username` and `<email>` if `config.publicEmails` === true
 *
 * @return {String} fullName
 * @api public
 */
...
UserSchema.virtual('hasExtProfile').get(function () {
  const { google, facebook, twitter, ldap } = this.profiles
  return !(isEmpty(google) && isEmpty(facebook) && isEmpty(twitter) && isEmpty(ldap))
})

UserSchema.virtual('updateOnNextLogin').get(function () {
  return this.profiles.updateOnNextLogin
})
