...
// var updateTopic = require('lib/api-v2/db-api/topics/utils').updateTopicFromComment
...
CommentSchema.methods.updateParticipation = function updateParticipation () {
  log('updateParticipation')
  var authors = function (a, b) {
    const aId = a.get ? a.get('_id') : has(a, '_id') ? a._id : a
    const bId = b.get ? b.get('_id') : has(b, '_id') ? b._id : b
    return aId.equals ? aId.equals(bId) : aId === bId
  }

  let voters = unique(pluck(this.votes.toObject(), 'author'), authors)
  this.count.voters = voters.length
  let repliers = unique(pluck(this.replies.toObject(), 'author'), authors)
...
// CommentSchema.post('save', updateTopic)
CommentSchema.post('save', function (data) {
  const _id = new mongoose.Types.ObjectId(data.reference)

  notifier.now('topic-stats', {
    topic: _id,
    comment: data
  })
  .then(() => {
