...
var log = require('debug')('democracyos:models:forum')
...
var ForumSchema = new Schema({
  associationId: { type: ObjectId, ref: 'Group' },
  creationReason: { type: String, required: true, trim: true },
  name: {
    type: String,
    required: true,
    trim: true,
  ...
  featuredSection: { type: Boolean, default: false },
...
ForumSchema.virtual('public').get(function () {
...
ForumSchema.methods.grantPermission = function (user, role, cb) {
  if (this.isOwner(user)) {
    ...
    log('-- silent error : The forum owner cannot be given another role.')
    return cb()
  }
