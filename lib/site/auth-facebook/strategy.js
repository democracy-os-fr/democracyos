...
module.exports = function () {
  const callbackURL = utils.buildUrl(config, {
    pathname: '/auth/facebook/callback',  port: 443, protocol: 'https'
  })
...
function assignProfile (user, profile, accessToken, fn) {
  try {
    ...
    if (profile._json.first_name) {
      ...
      names.push(profile._json.name.first_name)
    }

    if (profile._json.last_name) {
      user.set('lastName', profile._json.last_name)
      names.push(profile._json.name.last_name)
    }

    if (profile.displayName) {
      user.set('username', profile.displayName)
    } else if (profile._json.short_name) {
      user.set('username', profile._json.short_name)
    } else if (profile._json.full_name) {
      user.set('username', profile._json.full_name)
