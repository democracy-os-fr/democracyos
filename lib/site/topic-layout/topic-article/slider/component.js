...
class Slider extends Component {
  constructor (props) {
    super(props)
    this.state = {
      value: typecast(this.props.topic.voted) ? Number(this.props.topic.voted) : 0,
      voted: typecast(this.props.topic.voted),
      changingVote: false,
      colored: false
    }
  ...
  render () {
    const { value, changingVote, colored, voted } = this.state
    const { topic, user } = this.props

    const votesTotal = topic.action.count

    const showRange = !(topic.closed || voted) || changingVote
