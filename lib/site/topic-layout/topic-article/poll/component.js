...
export class Poll extends Component {
  ...
  setStateFromProps (props, _static = {}) {
    const { topic } = props
    const multiple = topic.action.multiple
    const ownVote = multiple ? JSON.parse(topic.voted) : topic.voted
    let voted = multiple ? JSON.parse(typecast(topic.voted)) : typecast(topic.voted)
  ...
  render () {
    ...
    const { results, changingVote, voted, selected } = this.state

    const votesTotal = this.props.topic.action.count

    if (!results) return null

    const showResults = topic.closed || (voted && !(topic.attrs && topic.attrs.hideResultsBeforeClosing) && !changingVote)
          ...
          )}
          {
            (showVoteBox && !showResults) && results.map((result, i) => (
              <Option
                ...
                value={result.value}
                selected={selected.includes(result.value)} />
            ))
          }
