...
class Hierarchy extends Component {
  ...
  prepareState (props) {
    const items = props.topic.action.results.map((item) => trim(item.value, ['"']))
    this.state = {
      value: typecast(props.topic.voted),
      voted: typecast(props.topic.voted),
      changingVote: false,
      items: typecast(props.topic.voted) ? items : shuffle(items)
    }
  }
  ...
  render () {
    const { value, changingVote, items, voted } = this.state
