...
class Vote extends Component {
  ...
  handleVote = (e) => {
    ...
    topicStore
      ...
      .then((res) => {
        if (this.resultChart) {
          this.resultChartDidMount(this.resultChart)
        }
        this.setState({
          voted: true,
          changingVote: false,
  ...
  render () {
    ...
    const showResult = (closed && votesTotal > 0) || !(this.state.topic.attrs && this.state.topic.attrs.hideResultsBeforeClosing)
    const showVoteBox = !closed && (!this.state.voted || this.state.changingVote)
    const showAlert = !(this.state.alert.hide || this.state.changingVote) && !closed
