...
class TopicArticle extends Component {
  ...
  componentWillReceiveProps (props) {
    ...
    this.setState({ showModal: false, keepSelection: false })
  }
  ...
  handleClose = () => {
    this.setState({ showModal: false, keepSelection: true })
  }

  handleChangePage = (e) => {
    e.preventDefault()
    const { topic } = this.props
    if ((topic.action.method === 'poll') && !this.canLeave) {
      this.setState({ showModal: true, keepSelection: true })
    } else {
      this.doChangePage()
    }
  }
  ...
  doChangePage = () => {
    window.location = this.nextLink.props.to
  }
          ...
          action = (
            <div className='topic-article-content'>
              <Poll
                ...
                canVoteAndComment={forum.privileges.canVoteAndComment}
                handleCanLeave={this.handleCanLeave} />
            </div>
          )
          break
        case 'cause':
        ...
        {nextTopic && (
          <div className='topic-article-content topic-social'>
            ...
            <Modal bsSize='small' className='topic-modal' backdrop
              ...
              <Modal.Body className='lead' dangerouslySetInnerHTML={{ __html: t('proposal-article.warning.leaving-without-vote') }} />
              <Modal.Footer>
                <Button bsSize='small' onClick={this.handleClose}>{t('common.cancel')}</Button>
                <Button bsSize='small' bsStyle='primary' onClick={this.doChangePage}>{t('common.ok')}</Button>
              </Modal.Footer>
            </Modal>
          </div>
        )}
