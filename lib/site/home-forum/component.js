...
export class HomeForum extends Component {
  ...
  render () {
    ...
    const canCreate = forum.privileges.canCreateTopics && user.privileges && user.privileges.canCreate

    if ((!user.privileges) && (['private', 'closed'].includes(forum.visibility))) {
      return browserHistory.push('/401')
    }
    ...
    return (
      <div id='forum-home'>
        ...
        {this.state.loading ? (
          ...
          <div>
            <FeaturedSection topics={topics} user={user} />
