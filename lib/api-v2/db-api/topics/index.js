...
exports.create = function create (opts, attrs) {
  ...
  switch (attrs['action.method']) {
    case 'vote':
      attrs['action.results'] = [template('positive'), template('neutral'), template('negative')]
      break
    case 'poll':
    case 'hierarchy':
      if (!attrs['action.options']) {
        return Promise.reject(new Error('Can\'t create a poll without options'))
      }
      attrs['action.results'] = attrs['action.options'].map((o) => (template(o)))
      delete attrs['action.options']
      break
    case 'cause':
      attrs['action.results'] = [template('support')]
      break
    case 'slider':
...
function doVote (user, value, topic) {
  if (topic.status === 'closed') {
    const err = new Error('Voting on this topic are closed.')
...
exports.copy = function copy (opts) {
  opts = opts || {}
  const source = opts.source
  const forum = opts.forum

  const query = {
    forum: new ObjectID(source)
  }
  return Promise.resolve()
    .then(() => Topic.collection.find(query).toArray())
    .then(mapPromises(function (topic) {
      topic.forum = forum
      topic._id = new ObjectID()

      // Results
      topic.action.results = topic.action.results.map(function (result) {
        return {
          value: result.value,
          percentage: 0,
          count: 0
      ...
      topic.action.count = 0

      // Attrs
      unset(topic, 'attrs.nextTopic')
