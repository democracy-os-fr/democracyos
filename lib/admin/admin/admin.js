...
import GeneralForm from '../admin-general/view'
import AdminComments from '../admin-comments/component'
import AdminUpload from '../admin-upload/upload'
...
page(urlBuilder.for('admin'), (ctx) => {
  page.redirect(urlBuilder.for('admin.general', { forum: ctx.forum.name }))
})
...
page(urlBuilder.for('admin.upload'), (ctx) => {
  ctx.sidebar.set('upload')
  render(<AdminUpload forum={ctx.forum} />, document.querySelector('.admin-content'))
})
page(urlBuilder.for('admin.general'), privileges('canEdit'), (ctx) => {
  // render topic form for edition
  let form = new GeneralForm(ctx.forum)
  form.replace('.admin-content')
  ctx.sidebar.set('general')
})
