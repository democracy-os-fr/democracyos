...
export default class TagsModeration extends Component {
  ...
  fetchTags = () => {
    ...
    request
    .get(`/api/v2/topics/tags?${query}`)
    .end((err, res) => {
      if (err) {
        console.error(err)
      } else {
        const { tags, pagination: { total, page, limit } } = res.body
        this.setState({
          tags,
          currentPage: page - 1,
          pages: (total / limit)
  ...
  selectTag = (index) => (e) => {
    ...
    if (contains(selectedTags, selectedTagText)) {
              ...
              tags.map(({ text, count }, i) => {
                return (
                  <tr
                    ...
                    className={contains(selectedTags, text) ? 'active' : ''}>
                    <td>{ text }</td>
                    <td>{ count }</td>
                  </tr>
                )
