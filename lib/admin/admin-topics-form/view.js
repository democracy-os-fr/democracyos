...
export default class TopicForm extends FormView {
  constructor (topic, forum, tags) {
    ...
    this.image = this.topic && this.topic.coverUrl ? {
      name: utils.imageName(this.topic.coverUrl),
      url: this.topic.coverUrl
    } : null
  ...
  onsaveclick (ev) {
    ev.preventDefault()
    if (this.find('.method-input')[0].value === 'poll' && this.find('.tags-input > span').length < 2 && this.locals.form.type === 'create') {
      this.messages(t('admin-topics-form.message.validation.pollOptions-required'), 'error')
      return
    }
    if (this.find('.method-input')[0].value === 'poll' && this.find('.tags-input > span').length < 4 && this.locals.form.type === 'edit' && this.topic.action.count === 0) {
      this.messages(t('admin-topics-form.message.validation.pollOptions-required'), 'error')
      return
    }
    if (this.find('.method-input')[0].value === 'hierarchy' && this.find('.tags-input > span').length < 2 && this.locals.form.type === 'create') {
      this.messages(t('admin-topics-form.message.validation.hierarchyOptions-required'), 'error')
      return
    }
    if (this.find('.method-input')[0].value === 'hierarchy' && this.find('.tags-input > span').length < 4 && this.locals.form.type === 'edit' && this.topic.action.count === 0) {
      this.messages(t('admin-topics-form.message.validation.hierarchyOptions-required'), 'error')
      return
    }

    // shaddy merge conditions ...
    if (this.find('.method-input')[0].value === 'poll' && this.find('.poll-options > input')[0].value === '') {
      this.messages(t('admin-topics-form.message.validation.pollOptions-required'), 'error')
      return
    }
    // -- END of : shaddy merge conditions ...
  ...
  postserialize (data = {}) {
    ...
    if (data['action.updatable']) {
      data['action.multiple'] = !!data['action.multiple']
    }
