...
page(urlBuilder.for('system.groups.update'), (ctx) => {
  render(<GroupUpdate {...ctx} />, document.querySelector('#system-content'))
})
...
page(urlBuilder.for('system.forums.create'), (ctx, next) => {
  if ((user.privileges && user.privileges.canCreate) || (config.multiForum)) return next()
  if (user) return page.redirect('/')
  else page.redirect('/signin?ref=%2Fsystem%2Fforums%2Fcreate')
}, (ctx) => {
