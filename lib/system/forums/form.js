...
import { contains, filter } from 'mout/array'
    ...
    if (isEmpty(e.delegateTarget.value)) {
      ...
      .end((err, res) => {
        if (err || !res.ok) {
          ...
          if (res.ok) {
            this.showElement(this.el[0].querySelector('.search'))

            if ((res.ok) && (res.text !== '[]')) {
              this.searchedGroup = filter(res.body, (group, key, arr) => {
                return contains(group.users, user.id)
              })
              ReactRender((
                <SearchResult isSearching groups={this.searchedGroup} selectGroup={this.select} />
