...
export default class GroupUpdate extends Component {
  ...
  render () {
    const { name, logoUrl, users, justificatoryUrl } = this.state
    let displayedUsers
    let savedUsers
    let title
    let nameErr
    ...
    return (
      <article className='group-add-form col-xs-12 col-md-8 col-md-offset-2'>
        ...
        <form
          ...
          <div className='JustificatoryUrlInput'>
            <label>{t('forum.form.justificatory')}</label>
            <input
              type='text'
              ...
              hidden />
            <ImageUploader
              pictures={this.state.file ? [this.state.file] : []}
              withCover
              singleImage
              withUrl
